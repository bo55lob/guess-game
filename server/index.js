...
io.on("connection", (socket) => {
    ...
    socket.on("createRoom", (playerName) => {
        ...
        rooms[roomCode] = { started:false, finished:false, currentTurn:0, currentQuestion:null, guessRevealActive:false, answers:[], questionHistory:[], players:[{id:socket.id,name:playerName,answer:"",ready:false,eliminated:false}] };
    ...
    socket.on("submitAnswer", ({roomCode,answer}) => {
        const room=rooms[roomCode]; const player=room?.players.find((p)=>p.id===socket.id);
        if(!room||!player||!answer?.trim()) return;
        player.answer=answer.trim(); player.ready=true; io.to(roomCode).emit("playersUpdated",room.players);
    });

    socket.on("startGame", (roomCode) => {
        const room=rooms[roomCode];
        if(!room||room.started||room.players.length<2||!room.players.every((p)=>p.ready)) return;
        room.started=true; room.currentTurn=0; io.to(roomCode).emit("gameStarted",{currentPlayer:room.players[room.currentTurn].name});
    });

    socket.on("askQuestion", ({roomCode,question}) => {
        const room=rooms[roomCode]; if(!room||!room.started||room.finished||room.guessRevealActive)return;
        ...
        room.currentQuestion={player:asker.name,question:question.trim(),answers:[]}; room.answers=[];
    ...
    socket.on("answerQuestion", ({roomCode,answer}) => {
        const room=rooms[roomCode]; if(!room||!room.currentQuestion||room.finished)return;
        const player=room.players.find((p)=>p.id===socket.id); const asker=room.players.find((p)=>p.name===room.currentQuestion.player);
        ...
        room.answers.push({player:player.name,answer}); room.currentQuestion.answers=room.answers;
        ...
        if(pendingPlayers.length===0){room.questionHistory.push({asker:asker.name,question:room.currentQuestion.question,answers:room.answers});io.to(roomCode).emit("questionHistory",room.questionHistory);room.currentQuestion=null;advanceTurn(room,roomCode);}
    });
    ...
    socket.on("makeGuess", ({roomCode,targetPlayer,guess}) => {
        ...
        setTimeout(()=>{
            ...
            if(activePlayers.length<=1){room.finished=true;if(activePlayers[0])io.to(roomCode).emit("gameWinner",{winner:activePlayers[0].name});return;}
            advanceTurn(room,roomCode);
        },3000);
    });

    socket.on("disconnect",()=>{for(const [roomCode,room] of Object.entries(rooms)){const index=room.players.findIndex((p)=>p.id===socket.id);if(index!==-1){room.players.splice(index,1);if(room.players.length===0)delete rooms[roomCode];else{if(room.currentTurn>=room.players.length)room.currentTurn=0;io.to(roomCode).emit("playersUpdated",room.players);}}}});

    function advanceTurn(room,roomCode){
        const activePlayers=room.players.filter((p)=>!p.eliminated); if(activePlayers.length<=1)return;
        const currentPlayerId=room.players[room.currentTurn]?.id; let index=activePlayers.findIndex((p)=>p.id===currentPlayerId);
        index=index===-1?0:(index+1)%activePlayers.length; room.currentTurn=room.players.findIndex((p)=>p.id===activePlayers[index].id); io.to(roomCode).emit("nextTurn",{player:activePlayers[index].name});
    }
});
